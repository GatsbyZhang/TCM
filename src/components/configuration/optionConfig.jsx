...
export  default class OptionConfig extends Component {
    componentDidMount() {
        $("#optionConfig-list").jqGrid(
            {
                ...
                colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                    ...
                    {name: 'operate', index: 'operate', width: "10%", align: "center",formatter: function (obj) {
                        if(obj){
                ...
                rowNum: 20,//一页显示多少条
                pgtext: '第{0}页   共 {1} 页',
                pager: '#optionConfig-page',//表格页脚的占位符(一般是div)的id
                sortname: 'name',//初始化的时候排序的字段
                sortorder: "asc",//排序方式,可选desc,asc
                viewrecords: true,
                gridComplete:function () {
                    $('button.cancel').click(function () {
                        let rowid = $("#optionConfig-list").jqGrid("getGridParam", "selrow")//所选择的行id
                        $("#optionConfig-list").jqGrid('delGridRow', rowid,{
                            left: 800,
                            top: 400,
                            caption: "撤销操作",
                            msg: "你确定要撤销此操作?",
        ...
        $(window).resize(function () {//表格自适应屏幕
            $("#optionConfig-list").setGridWidth($("#configuration-cart").width() - 10)
        });
    }
        ...
        return (
            <div className="configuration">
                ...
                <div id="configuration-cart">
                    <table id="optionConfig-list"  className="option-record"></table>
                    <div id="optionConfig-page"></div>
                </div>
            </div>
        )
    }
