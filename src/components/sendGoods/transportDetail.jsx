...
class TaskDetail extends Component {
    ...
    handleCancel() {
        this.props.actions.closeTransportDetailModelList();
    }

    render() {
        let state = this.props.sendGoodsTodos;
        let aDatas = state.compactData.rows;
        let imgUrl = '/static/img/transport.png';
        let data = aDatas.filter(item=> {
            return item.id == state.transportId;
        });
        return <div>
            <Modal title="任务详情" visible={state.transportVisible} width="1200px"
                    ...
                    return <Form key={item.id} style={{width:'100%'}}>
                        <Row>
                           <Col span={5}>
                            ...
                            <Col span={5}>
                                 <img src={imgUrl} style={{width:'680px',height:'100px'}} />
                            </Col>
                        </Row>
                        <Row>
                            <Col span={12}>
