...
class Tab8 extends Component {
    componentDidMount(){
        ...
        let that=this;
        $("#sendGoods-list8").jqGrid({
            datatype: "local", //数据来源，本地数据
            data: compactData,
            mtype: "GET",//提交方式
            height: " auto",//高度，表格高度。可为数值、百分比或'auto'
            autowidth: true,//自动宽
            colNames: ['序号', '姓名', '电话','在线时长','累计车数','累计方数'],
            colModel: [
                //{name:'id',index:'id', width:'10%', align:'center' },
                {name: 'id', index: 'id', width: '10%', align: 'center',sortable: false},
                {name: 'name', index: 'name', width: '20%', align: 'center',sortable: false},
                {name: 'telNumber', index: 'telNumber', width: '35%', align: 'center',sortable: false},
                {name: 'hour', index: 'hour', width: '15%', align: "center",sortable: false},
                {name: 'cCount', index: 'cCount', width: '15%', align: "center",sortable: false},
                {name: 'aFCount', index: 'aCount', width: '15%', align: "center", sortable: false,}
            ],
            ...
            pager: $('#sendGoods-gridPager8'),
            ondblClickRow:function(id, iRow, iCol, e){
                let row = $("#sendGoods-list8").jqGrid('getLocalRow', id);//返回原始json数据
                if(iCol!=7){
                    that.props.actions.toggleRegisterModelList(row.id);
                }
            },
        });
    ...
    render() {
        return <div>
            <table id="sendGoods-list8">
            </table>
            <div id="sendGoods-gridPager8"></div>
        </div>
    }
}
export default Tab8;